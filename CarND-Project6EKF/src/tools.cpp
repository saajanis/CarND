...
VectorXd Tools::PolarToCartesian(const VectorXd& v) {

  VectorXd cartesian(4);

  const double rho = v(0);
  const double phi = v(1);
  const double rhoDash = v(2);

  const double px = rho * cos(phi);
  const double py = rho * sin(phi);
  const double vx = rhoDash * cos(phi);
  const double vy = rhoDash * sin(phi);
