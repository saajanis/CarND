...
std::map<string, vector<double>> MPC::Solve(Eigen::VectorXd state, Eigen::VectorXd coeffs) {
    bool ok = true;
    size_t i;
    ...
    // Use the second value instead of first for actuation to account for 100 ms latency
    ...
    mpc_y_vals.clear();
    for (int x=0; x < N-1; x++) {
        mpc_x_vals.push_back(solution.x[x_start + 1 + x]);
        mpc_y_vals.push_back(solution.x[y_start + 1 + x]);
    }

    std::map<string, vector<double>> sol_map = {
            { "next_state", next_state },
            { "mpc_x_vals", mpc_x_vals },
            { "mpc_y_vals", mpc_y_vals }
    };
