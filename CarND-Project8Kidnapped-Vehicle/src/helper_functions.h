...
inline int closestValueIndex(std::vector<double> const& vec, int value) {
	// TODO: Reconsider what happens when -1 is returned?
	std::vector<double> vec_copy = std::vector<double>(vec);
	std::sort(vec_copy.begin(), vec_copy.end());
	std::vector<double> sorted_vec = vec_copy;

	// find closest value
	auto const it = std::lower_bound(sorted_vec.begin(), sorted_vec.end(), value);
	if (it == sorted_vec.end()) { return -1; }

	// find index of closest value
	auto match = std::find(vec.begin(), vec.end(), *it);
	if(match != vec.end()) {
		return match - vec.begin();
	} else {
		return -1;
	}
}
