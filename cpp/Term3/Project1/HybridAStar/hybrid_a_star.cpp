...
double HAS::euclidean(vector<double> state, vector<double> goal){
    double x = state[0] - goal[0];
    double y = state[1] - goal[1];
...
vector<HAS::maze_s> HAS::expand(HAS::maze_s state, vector<double> goal) {
    int g = state.g;
    double x = state.x;
    double y = state.y;
    ...
    for(double delta_i = -35; delta_i < 40; delta_i+=5)
    {
        ...
        double f2 = g + euclidean({state.x, state.y}, goal);
...
vector< HAS::maze_s> HAS::reconstruct_path(vector< vector< vector<HAS::maze_s> > > came_from, vector<double> start, HAS::maze_s final){
...
HAS::maze_path HAS::search(vector< vector<int> > grid, vector<double> start, vector<int> goal) {
    /*
    ...
    */
    ...
    while(!opened.empty())
    {
        sort(opened.begin(), opened.end(), [](const maze_s &a, const maze_s &b) -> bool { return a.f < b.f;});
        ...
        vector<maze_s> next_state = expand(current, {(double)goal[0], (double)goal[1]});
